...
compile_error!("At least one of the features of mysql must be enabled, since no other adapter has been implemented.");
...
pub struct Connection {
    ...
    #[cfg(any(feature = "time-mysql", feature = "chrono-mysql"))]
    threads: std::collections::VecDeque<(usize, std::thread::JoinHandle<Result<(), ::mysql::Error>>)>,
...
trait DatabaseConnection {
    /// The error returned by the mysql library used.
    type Error: std::error::Error;

    /// Creates the FinalConfiguration table of the database.
    ///
    /// # Errors
    /// Will return [`DatabaseConnection::Error`] if the creation fails.
    ...
    ///
    /// # Errors
    /// Will return [`DatabaseConnection::Error`] if any of the creations fails.
    fn create_views(&mut self) -> Result<&mut Self, Self::Error>;

    /// Creates the lichess database, selects it and creates all its tables.
    ///
    /// # Errors
    /// Will return [`DatabaseConnection::Error`] if the creation or selection fails.
...
pub trait DatabaseAdapter: Sized {
    /// The error returned by the mysql library used.
    type Error: std::error::Error;
    /// The pooled connection to use in inserts.
    type PoolConn;
    ...
    /// Will return [`DatabaseAdapter::Error`] if the insertion fails.
    #[allow(dead_code)]
    fn insert_move(conn: &mut Self::PoolConn, r#move: &Move, game_id: u64) -> Result<(), Self::Error>;

    /// Inserts a [`Vec`] of [`Move`]s into the Move table.
    ///
    /// # Errors
    /// Will return [`DatabaseAdapter::Error`] if the insertion fails.
    fn insert_moves(conn: &mut Self::PoolConn, moves: &[Move], game_id: u64)
    -> Result<(), Self::Error>;
