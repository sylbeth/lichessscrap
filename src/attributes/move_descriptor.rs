//! Represents a move made in a Lichess game. It holds Nag, suffix, and the whole characterization of the move.
...
impl MoveDescriptor {
    ...
    /// `player` (1 bit) - 0 for the black player, 1 for the white player.
    /// `moved_role` (3 bits) - The role of the moved piece.
    /// `starting_square` (6 bits) - The starting square of the move.
    /// `captured_role` (3 bits) - The role of the captured piece. 7 for En-Passant.
    /// `ending_square` (6 bits) - The ending square of the move, or the rook's square if it's a castling.
    /// `promoted_role` (3 bits) - The role of the promoted piece. 6 and 7 for castling (King and Queen side, respectively).
    /// `is_check` (1 bit) - Whether it's check or not.
    /// `is_checkmate` (1 bit) - Whether it's checkmate or not.
    /// `nag` (8 bits) - The value of the nag.
